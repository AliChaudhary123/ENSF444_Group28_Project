...
def _build_regression_df(sector: str, community: str, category: str,
    ...
    df = pd.DataFrame({
        ...
        "month": str(month),  # Keep month as string for regression model
        "resident_count": float(resident_count),  # Convert to float for regression model
    })
    return df
...
def _build_classification_df(sector: str, community: str,
    ...
    df = pd.DataFrame({
        ...
        "community_name": pd.array([community], dtype="object"),
        "year": np.array([year], dtype="int64"),
        "month": str(month),  # Keep month as string for classification model
        "resident_count": float(resident_count),  # Convert to float for classification model
    })
    return df
...
def predict_classification(model_name: str, community: str, sector: str,
    ...
    return {
        ...
        "model_used": model_name,
        "task": "classification",
        "inputs": {"sector": sector, "community": community, "year": year, "month": month, "resident_count": resident_count},
    }
